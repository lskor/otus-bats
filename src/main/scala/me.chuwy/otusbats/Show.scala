...
object Show {
  ...
  implicit def intToString = fromJvm[Int]
  implicit def stringToString = fromJvm[String]
  implicit def booleanToString = fromJvm[Boolean]

  // 1.2 Instances with conditional implicit

  implicit def listShow[A](implicit ev: Show[A]): Show[List[A]] =
    fromFunction[List[A]](list => Show.mkString_(list.map(_.show), "List[", "]", ""))
